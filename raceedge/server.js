...
async function initDb() {
  if (!pool) return;
  await pool.query(`
    CREATE TABLE IF NOT EXISTS raceedge_tip_results (
...
function scoreRunner(r = {}) {
  ...
  const val = (key, fallback = 50) => Number.isFinite(Number(r[key])) ? Number(r[key]) : fallback;
  return Math.round(Object.entries(weights).reduce((sum, [key, weight]) => sum + val(key) * weight, 0));
}
...
app.post('/api/v1/rating', (req, res) => {
  res.json({
    score: scoreRunner(req.body),
    label: 'prototype-rating',
    backtested: false,
    weights: { form: 0.25, speed: 0.20, class: 0.15, pace: 0.15, conditions: 0.15, barrier: 0.10 }
  });
});
...
app.get('/api/v1/provider/next-to-go', async (_req, res) => {
  ...
  if (!key) {
    return res.status(503).json({
      connected: false,
      message: 'Live racing provider key not configured yet.',
      demo
    });
  }
  ...
  try {
    ...
    const response = await fetch(`${base}/v1/racing/next-to-go`, {
      headers: {
        'X-API-Key': key,
    ...
    console.error('PuntersEdge request failed', error);
    res.status(502).json({ connected: false, error: 'Provider request failed' });
  }
});

app.get('/api/v1/results', async (_req, res) => {
  if (!pool) return res.json([]);
  const { rows } = await pool.query('SELECT * FROM raceedge_tip_results ORDER BY created_at DESC LIMIT 100');
  res.json(rows);
});
...
app.post('/api/v1/results', async (req, res) => {
  ...
  if (!meeting || !Number.isInteger(Number(raceNo)) || !runner) {
    return res.status(400).json({ error: 'meeting, raceNo and runner are required' });
  }

  const { rows } = await pool.query(
    'INSERT INTO raceedge_tip_results(meeting,race_no,runner,score,price,result_position) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',
    [meeting, Number(raceNo), runner, score ?? null, price ?? null, resultPosition ?? null]
  );
...
initDb()
  .then(() => app.listen(port, '0.0.0.0', () => console.log(`RaceEdge listening on ${port}`)))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
