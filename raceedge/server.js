...
const demoField = [
  {number:4,name:'Northern Edge',barrier:3,barrierRating:90,price:3.4,scratched:false,form:94,speed:91,classRating:88,pace:92,conditions:90},
  {number:7,name:'Ocean State',barrier:6,barrierRating:76,price:6.5,scratched:false,form:86,speed:84,classRating:87,pace:82,conditions:88},
  {number:2,name:'Capital Run',barrier:1,barrierRating:94,price:4.8,scratched:false,form:83,speed:87,classRating:82,pace:84,conditions:80},
  {number:9,name:'Trackside',barrier:8,barrierRating:62,price:9.0,scratched:true,form:89,speed:86,classRating:81,pace:79,conditions:85}
].map(r => ({...r, barrier:r.barrierRating}));
...
async function initDb(){
 if(!pool) return;
 await pool.query(`CREATE TABLE IF NOT EXISTS raceedge_tip_results (
 ...
 await pool.query(`ALTER TABLE raceedge_tip_results ADD COLUMN IF NOT EXISTS stake NUMERIC DEFAULT 1`);
 await pool.query(`ALTER TABLE raceedge_tip_results ADD COLUMN IF NOT EXISTS settled_return NUMERIC`);
 await pool.query(`ALTER TABLE raceedge_tip_results ADD COLUMN IF NOT EXISTS code TEXT`);
 await pool.query(`CREATE TABLE IF NOT EXISTS raceedge_published_tips (
   id BIGSERIAL PRIMARY KEY,
   meeting TEXT NOT NULL,
   race_no INT NOT NULL,
   runner TEXT NOT NULL,
...
app.get('/health',async(_req,res)=>{let database='not-configured';if(pool){try{await pool.query('SELECT 1');database='ok';}catch{database='error';}}res.json({ok:true,service:'raceedge',version:'0.4.0',database,providerConfigured:providerConfigured(),time:new Date().toISOString()});});
