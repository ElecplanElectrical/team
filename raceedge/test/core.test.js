...
const complete = (overrides={}) => ({ form:90,speed:88,classRating:84,pace:86,conditions:82,barrierRating:80,...overrides });

test('prototype rating returns a bounded score with complete rating factors', () => {
  const result = scoreRunner(complete());
...
test('barrier position alone is not treated as a barrier performance rating', () => {
  const result = scoreRunner({ form:90,speed:88,classRating:84,pace:86,conditions:82,barrier:2 });
  assert.equal(result.coverage.missingComponents.includes('barrier'), true);
  assert.equal(result.coverage.weightedCoverage, 0.9);
});

test('scratched runners are excluded from selections', () => {
  const rated = rateField([
    complete({ number:1,name:'A',scratched:true }),
    complete({ number:2,name:'B',scratched:false }),
    complete({ number:3,name:'C',scratched:false,form:70,speed:70 })
  ]);
  ...
  assert.equal(rated.analysisReady, true);
});

test('incomplete component data suppresses fair price, value and confidence', () => {
  const rated = rateField([
    { number:1,name:'A',scratched:false,price:3.0,barrier:2 },
    { number:2,name:'B',scratched:false,price:4.0,barrier:6 }
  ]);
