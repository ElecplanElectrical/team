...
@MainActor final class RaceEdgeAPI: ObservableObject {
 ...
 func loadHome() async {
   ...
   async let liveRequest: LiveToday? = try? fetchJSON("/api/v1/live/today",as:LiveToday.self)
   async let homeRequest: RaceEdgeHome? = try? fetchJSON("/api/v1/home",as:RaceEdgeHome.self)
   let(live,homePayload)=await(liveRequest,homeRequest)
   if let live { liveToday=live }
   if let homePayload { home=homePayload }
   if live == nil && homePayload == nil { errorMessage="RaceEdge data is temporarily unavailable." }
 }
...
func raceEdgeTimeText(_ raw: String?) -> String? {
