...
export default async function SettingsPage() {
  ...
  const xeroConnection = xero.configured ? await prisma.xeroConnection.findUnique({ where: { id: "elecplan" }, select: { tenantName: true, lastSyncAt: true } }) : null;
  ...
  const checks = [
    ...
    { label: "Xero contact import", ready: Boolean(xeroConnection), detail: xeroConnection ? `Read-only customer import connected to ${xeroConnection.tenantName}.` : "Connect the Elecplan Xero organisation from Clients after credentials are configured." },
  ];
    ...
    <div className="flex-1 overflow-auto p-3 md:p-4 xl:p-5" style={{ background: "var(--ep-main)" }}>
      <div className="mx-auto w-full max-w-[1700px] space-y-4">
        ...
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {cards.map(({ href, icon: Icon, title, detail }) => <Link key={href} href={href} className="group rounded-xl p-4 transition hover:-translate-y-0.5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "rgba(67,210,255,.11)", color: UI.cyan }}><Icon size={18} /></span><div className="min-w-0"><h2 className="text-sm font-semibold" style={{ color: UI.text }}>{title}</h2><p className="mt-1 text-xs leading-5" style={{ color: UI.mute }}>{detail}</p></div></div></Link>)}
        </section>
        ...
        <section className="grid gap-3 xl:grid-cols-2">
          <div className="rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "rgba(67,210,255,.11)", color: UI.cyan }}><FileText size={18} /></span><div><h2 className="text-sm font-semibold" style={{ color: UI.text }}>Security audit trail</h2><p className="mt-2 text-xs leading-5" style={{ color: UI.mute }}>Review sensitive finance, access, SMS and approval activity. Audit metadata excludes secret values and SMS message bodies.</p><Link href="/settings/audit" className="mt-4 inline-flex rounded-lg px-3 py-2 text-xs font-semibold" style={{ ...UI.inset, background: UI.panelAlt, border: `1px solid ${UI.borderSoft}`, color: UI.cyan }}>Open audit log</Link></div></div></div>

          <div className="rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "rgba(67,210,255,.11)", color: UI.cyan }}><PlugZap size={18} /></span><div><h2 className="text-sm font-semibold" style={{ color: UI.text }}>Xero client import</h2><p className="mt-2 text-xs leading-5" style={{ color: UI.mute }}>Elecplan can now pull customer contacts into Clients without sending Elecplan data back to Xero. Tokens are encrypted and bound to the selected Elecplan organisation. Invoice, payment and financial write-back remain disabled.</p><span className="mt-4 inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold" style={{ background: xeroConnection ? "rgba(25,211,162,.10)" : "rgba(255,159,28,.10)", color: xeroConnection ? UI.green : UI.orange, border: `1px solid ${xeroConnection ? "rgba(25,211,162,.24)" : "rgba(255,159,28,.24)"}` }}>{xeroConnection ? `Connected · ${xeroConnection.tenantName}` : "Needs connection"}</span></div></div></div>
        </section>

        {(missingLicence.length > 0 || expiringLicence.length > 0) && <section className="rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: "1px solid rgba(255,94,114,.24)" }}><h2 className="text-sm font-semibold" style={{ color: UI.text }}>Team compliance attention</h2><div className="mt-3 space-y-2">{missingLicence.map((member) => <div key={`missing-${member.id}`} className="text-xs" style={{ color: UI.red }}>{member.name}: licence number missing</div>)}{expiringLicence.map((member) => <div key={`expiry-${member.id}`} className="text-xs" style={{ color: UI.red }}>{member.name}: licence expires {member.licenseExpiry?.toLocaleDateString("en-AU")}</div>)}</div><Link href="/employees" className="mt-4 inline-flex text-xs font-semibold" style={{ color: UI.cyan }}>Open Employees →</Link></section>}
      </div>
    </div>
  </>;
}
...
function Metric({ label, value, warn = false }: { label: string; value: string; warn?: boolean }) {
  return <div className="rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="text-[11px]" style={{ color: UI.faint }}>{label}</div><div className="mt-1 text-xl font-semibold" style={{ color: warn ? UI.red : UI.text }}>{value}</div></div>;
}
