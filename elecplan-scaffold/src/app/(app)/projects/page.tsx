...
export default async function ProjectsPage() {
  ...
  const [photos, jobs] = await Promise.all([
    prisma.projectPhoto.findMany({ where: { job: { businessId } }, orderBy: { uploadedAt: "desc" }, select: { id: true, fileUrl: true, uploadedAt: true, job: { select: { title: true, address: true, client: { select: { name: true } } } } } }),
