...
export default async function HqPage(){
 ...
 <section className="grid gap-3 lg:grid-cols-2"><LaunchCard title="YourPlan HQ" text="Platform-only admin controls for customers, plans, modules and access." status="Admin protected" /><LaunchCard title="QLS · Customer #1" text={qls?`${qls.name} · ${qls._count.users} user${qls._count.users===1?"":"s"} · ${subMap.get(qls.id)?.status??"ACTIVE"}`:"QLS tenant not found"} status={qls?.active?"Tenant active":"Needs attention"} /></section>
 <nav className="grid gap-3 sm:grid-cols-2"><a href="#payments" className="rounded-xl px-4 py-3 text-sm font-semibold" style={{background:UI.panel,border:`1px solid ${UI.border}`,color:UI.cyan}}>Manage customer payments</a><a href="#onboarding" className="rounded-xl px-4 py-3 text-sm font-semibold" style={{background:UI.panel,border:`1px solid ${UI.border}`,color:UI.cyan}}>Onboard a new customer</a></nav>
 ...
 <div id="onboarding" className="scroll-mt-20"><BusinessPortalForm/></div><div id="customers" className="scroll-mt-20"><BusinessPortalManager businesses={businesses}/></div>
