...
export default async function InvoicesPage(){
  ...
  return <>
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6" style={{background:"var(--app-bg, #03101f)"}}>
      <div className="mx-auto w-full max-w-[1500px] space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl p-4" style={{background:"var(--brand-panel, #081b30)",border:"1px solid var(--brand-border, rgba(77,150,221,.23))"}}><p className="text-xs text-slate-400">Total invoiced</p><p className="mt-1 text-xl font-semibold text-white">{money(total)}</p></div>
          <div className="rounded-xl p-4" style={{background:"var(--brand-panel, #081b30)",border:"1px solid var(--brand-border, rgba(77,150,221,.23))"}}><p className="text-xs text-slate-400">Outstanding</p><p className="mt-1 text-xl font-semibold text-white">{money(outstanding)}</p></div>
        </div>
        <div className="overflow-hidden rounded-xl" style={{background:"var(--brand-panel, #081b30)",border:"1px solid var(--brand-border, rgba(77,150,221,.23))"}}>
