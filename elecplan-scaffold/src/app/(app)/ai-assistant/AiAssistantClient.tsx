...
type PortalAction={action:"ADD_JOB_TASK"|"USE_MATERIAL"|"COMPLETE_JOB"|"SEND_CONFIRMATION"|"INVOICE_ACCEPTED_QUOTE";jobId?:string;materialId?:string;quoteId?:string;quantity?:number;title?:string;notes?:string;label:string;detail:string;selected:boolean};
...
export default function AiAssistantClient(){
 const [message,setMessage]=useState(""),[file,setFile]=useState<File|null>(null),[items,setItems]=useState<Proposal[]>([]),[actions,setActions]=useState<PortalAction[]>([]),[answer,setAnswer]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState(false),[listening,setListening]=useState(false);
 ...
 const selected=items.filter(x=>x.selected).length,selectedActions=actions.filter(x=>x.selected).length;
 function choose(next?:File){if(!next)return;if(next.size>12*1024*1024){setError("That file is too large. Maximum 12 MB.");return}setFile(next);setItems([]);setActions([]);setAnswer("");setError("")}
 ...
 async function send(text=message,ignoreFile=false){if(busy||(!text.trim()&&!file))return;setBusy(true);setError("");setAnswer("");setItems([]);setActions([]);try{
 if(!file&&text.trim()){
  const plan=await fetch("/api/ai-assistant/plan-action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({instruction:text.trim()}),cache:"no-store"});
  const planned=await plan.json();
  if(plan.ok&&Array.isArray(planned.planned)&&planned.planned.length){setAnswer(planned.summary||"Review the action below.");setActions(planned.planned.map((x:PortalAction)=>({...x,selected:true})));return}
 }
 const fd=new FormData();if(file&&!ignoreFile)fd.append("file",file);fd.append("instruction",text.trim()||"Read this and propose anything actionable. Do not add it until I approve.");const res=await fetch("/api/ai-assistant/analyse",{method:"POST",body:fd,cache:"no-store"});const data=await res.json();if(!res.ok)throw new Error(data.error||"Could not complete that request.");setAnswer(data.summary||"Check the result below.");setItems((data.proposals||[]).map((x:Proposal)=>({...x,selected:x.kind==="reminder"||Boolean(x.startsAt)})))}catch(e){setError(e instanceof Error?e.message:"Could not complete that request.")}finally{setBusy(false)}}
 async function applyActions(){const chosen=actions.filter(x=>x.selected);if(!chosen.length)return;setBusy(true);setError("");try{for(const action of chosen){const {label,detail,selected,...payload}=action;void label;void detail;void selected;const res=await fetch("/api/ai-assistant/actions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok)throw new Error(data.error||"Could not apply that portal action.")}setActions([]);setMessage("");setAnswer(`Done — ${chosen.length} portal action${chosen.length===1?"":"s"} completed.`)}catch(e){setError(e instanceof Error?e.message:"Could not apply that portal action.")}finally{setBusy(false)}}
 ...
 return <div className={styles.page}><div className={styles.wrap}>
  ...
  {actions.length>0&&<section className={styles.review}><h2>Review portal action</h2>{actions.map((x,i)=><div className={styles.proposal} key={i}><input type="checkbox" checked={x.selected} onChange={()=>setActions(v=>v.map((a,n)=>n===i?{...a,selected:!a.selected}:a))}/><div><strong>{x.label}</strong><small>{x.detail}</small></div><button type="button" onClick={()=>setActions(v=>v.filter((_,n)=>n!==i))}><X/></button></div>)}<button className={styles.apply} disabled={!selectedActions||busy} onClick={()=>void applyActions()}>Approve and run {selectedActions||"selected"}</button></section>}
