...
import { Building2, CircleDollarSign, Power } from "lucide-react";
...
export default async function PlatformPage(){
 ...
 const rows=await prisma.businessPortal.findMany({orderBy:{createdAt:"desc"}});
 const active=rows.filter(b=>b.active); const mrr=active.reduce((sum,b)=>sum+Number(b.monthlyPrice??0),0);
 ...
 <section className="grid gap-3 sm:grid-cols-3"><Metric icon={<Building2 size={17}/>} label="Businesses" value={String(rows.length)}/><Metric icon={<Power size={17}/>} label="Active subscriptions" value={String(active.length)}/><Metric icon={<CircleDollarSign size={17}/>} label="Monthly recurring" value={new Intl.NumberFormat("en-AU",{style:"currency",currency:"AUD",maximumFractionDigits:0}).format(mrr)}/></section>
