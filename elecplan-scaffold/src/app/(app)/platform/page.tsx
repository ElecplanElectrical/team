...
import { Activity, Building2, CircleDollarSign, ExternalLink, Power, ShieldCheck, Users } from "lucide-react";
...
export default async function PlatformPage(){
 ...
 <section className="grid gap-3 lg:grid-cols-3">
  <LaunchCard title="YourPlan Demo" text="Public, read-only sales demo using sample data only." status="Live route" href="/demo" />
...
function LaunchCard({title,text,status,href}:{title:string;text:string;status:string;href?:string}){return <div className="rounded-xl p-4" style={{background:UI.panel,border:`1px solid ${UI.border}`}}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><ShieldCheck size={17} style={{color:UI.green}}/><span className="text-sm font-semibold" style={{color:UI.text}}>{title}</span></div>{href?<Link href={href} className="rounded-lg border px-2 py-1" style={{borderColor:UI.border,color:UI.cyan}}><ExternalLink size={14}/></Link>:null}</div><p className="mt-3 text-xs leading-5" style={{color:UI.mute}}>{text}</p><div className="mt-3 text-xs font-medium" style={{color:UI.green}}>{status}</div></div>}
