    ...
    <div className="flex-1 overflow-auto p-3 md:p-4 xl:p-5" style={{ background: "var(--ep-main)" }}>
      <div className="mx-auto grid w-full max-w-5xl gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <section className="rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}>
          ...
          <div className="mt-5 rounded-xl p-3 text-xs leading-5" style={{ ...UI.inset, background: UI.panelAlt, border: `1px solid ${UI.borderSoft}`, color: UI.mute }}>Your access level is controlled by your Elecplan team role. Contact an administrator if your role or account details need changing.</div>
        </section>
...
function Detail({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex gap-3 rounded-lg p-3" style={{ ...UI.inset, background: UI.panelAlt, border: `1px solid ${UI.borderSoft}` }}><span className="mt-0.5" style={{ color: UI.cyan }}>{icon}</span><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[.10em]" style={{ color: UI.faint }}>{label}</p><p className="mt-1 break-words text-sm" style={{ color: UI.text }}>{value}</p></div></div>; }
