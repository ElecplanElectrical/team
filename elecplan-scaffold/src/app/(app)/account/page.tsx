...
export default async function AccountPage() {
  ...
  return <>
    <TopBar title="Account" subtitle={`Your ${businessName} profile and sign-in security`} />
    <div className="flex-1 overflow-auto p-3 md:p-4 xl:p-5" style={{ background: "radial-gradient(circle at 55% 0%,var(--brand-glow, rgba(20,91,160,.12)),transparent 35%),var(--app-bg, #03101f)" }}>
      <div className="mx-auto grid w-full max-w-5xl gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <section className="rounded-xl p-5" style={{ background: UI.panel, border: `1px solid ${UI.border}` }}>
          ...
          <div className="mt-5 rounded-xl p-3 text-xs leading-5" style={{ background: UI.panelAlt, border: `1px solid ${UI.borderSoft}`, color: UI.mute }}>Your access level is controlled by your {businessName} team role. Contact an administrator if your role or account details need changing.</div>
        </section>
