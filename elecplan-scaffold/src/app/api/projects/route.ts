...
export async function POST(req: Request) {
  ...
  const id = randomUUID();
  const photo = await prisma.projectPhoto.create({ data: { id, jobId: job.id, fileUrl: `/api/projects/${id}/file`, storageKey: upload.key, originalName: upload.fileName, contentType: upload.contentType, sizeBytes: upload.sizeBytes } });
