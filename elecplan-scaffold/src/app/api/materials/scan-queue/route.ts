...
const schema=z.object({barcode:z.string().min(1).max(64),dataUrl:z.string().startsWith("data:image/jpeg;base64,").max(1800000),itemName:z.string().max(160).optional(),selection:selectionSchema});
...
export async function POST(req:Request){
  ...
  if(!item){
    ...
    item=await prisma.stockItem.create({data:{name:taughtName,unit:"each",onHand:0,parLevel:0,barcode,barcodes:{create:{barcode}}}});
  }else if(!alias){
    await prisma.stockBarcode.upsert({where:{barcode},create:{barcode,stockItemId:item.id},update:{stockItemId:item.id}});
  }

  const job=await prisma.$transaction(async tx=>{
    if(taughtName&&item!.name!==taughtName)await tx.stockItem.update({where:{id:item!.id},data:{name:taughtName}});
