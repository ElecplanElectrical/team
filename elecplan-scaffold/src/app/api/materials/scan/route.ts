...
export async function POST(req: Request) {
  ...
  // Quantity is ALWAYS supplied by the current physical box scan.
  // We never remember/reuse quantity for a barcode.
  ...
  if (found) {
    await prisma.stockBarcode.upsert({
      where: { barcode },
      create: { barcode, stockItemId: found.id },
      update: { stockItemId: found.id },
    });
  ...
  // First ever scan of this barcode: the selected label text becomes the item identity.
  const name = String(body.name ?? "").trim().slice(0, 160);
  if (!name) {
    return NextResponse.json({ error: "Select the item name on the label first" }, { status: 409 });
  }
