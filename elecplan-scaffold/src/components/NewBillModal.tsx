...
export default function NewBillModal({ clients, jobs, onClose, onDone }: { clients: BillClientOption[]; jobs: BillJobOption[]; onClose: () => void; onDone: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-0 backdrop-blur-sm md:items-center md:p-4" onClick={onClose} role="presentation">
    <section className="w-full max-w-2xl overflow-hidden rounded-t-2xl md:rounded-2xl" style={{ background: UI.panel, border: `1px solid ${UI.border}`, boxShadow: "0 28px 90px rgba(0,0,0,.35)" }} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="new-bill-title">
      <header className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: UI.borderSoft }}>
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "rgba(22,141,255,.11)", color: UI.cyan }}><ReceiptText size={18} /></span>
        <div className="min-w-0 flex-1"><h2 id="new-bill-title" className="text-base font-semibold" style={{ color: UI.text }}>New bill / invoice</h2><p className="mt-1 text-xs" style={{ color: UI.faint }}>Record a client receivable or supplier payable.</p></div>
      ...
      <form onSubmit={submit} className="max-h-[82vh] overflow-auto p-5" aria-busy={saving}>
        <div className="grid grid-cols-2 gap-2 rounded-xl p-1" style={{ background: "#041323", border: `1px solid ${UI.borderSoft}` }}>
          <button type="button" onClick={() => switchKind("client")} className="rounded-lg px-3 py-2.5 text-sm font-semibold" style={{ background: kind === "client" ? UI.blue : "transparent", color: kind === "client" ? "white" : UI.mute }} aria-pressed={kind === "client"}>Client invoice</button>
          <button type="button" onClick={() => switchKind("supplier")} className="rounded-lg px-3 py-2.5 text-sm font-semibold" style={{ background: kind === "supplier" ? UI.blue : "transparent", color: kind === "supplier" ? "white" : UI.mute }} aria-pressed={kind === "supplier"}>Supplier bill</button>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Amount (AUD)"><input type="number" min="0.01" step="0.01" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="h-11 w-full rounded-lg px-3 text-sm outline-none" style={field} /></Field>
