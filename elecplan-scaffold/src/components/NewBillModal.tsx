...
export default function NewBillModal({ clients, jobs, storageReady, onClose, onDone }: { clients: BillClientOption[]; jobs: BillJobOption[]; storageReady: boolean; onClose: () => void; onDone: () => void }) {
  ...
  async function uploadToPrivateStorage(): Promise<string | null> {
    ...
    try {
      ...
      return uploadResponse.ok ? ticket.commitToken : null;
    } catch {
      return null;
    }
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const documentCommitToken = await uploadToPrivateStorage();
      const documentDataBase64 = file && !documentCommitToken ? await fileBase64(file) : null;
      ...
      const response = await fetch("/api/bills", {
        ...
        body: JSON.stringify({
          ...
          documentContentType: file?.type || null,
  ...
  const field = { background: "#0d1117", border: `1px solid ${UI.border}`, color: UI.text } as const;
    ...
    <section className="w-full max-w-3xl overflow-hidden rounded-t-2xl md:rounded-2xl" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}`, boxShadow: "0 28px 90px rgba(0,0,0,.35)" }} onClick={(event) => event.stopPropagation()}>
      ...
      <form onSubmit={submit} className="max-h-[84vh] overflow-auto p-5">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Invoice / statement number"><input value={invoiceNumber} onChange={(event) => setInvoiceNumber(event.target.value)} placeholder="Optional" className="h-11 w-full rounded-lg px-3 text-sm outline-none" style={field} /></Field>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="rounded-lg px-4 py-2.5 text-sm font-semibold" style={{ ...UI.inset, background: UI.panelAlt, color: UI.mute, border: `1px solid ${UI.borderSoft}` }}>Cancel</button><button type="submit" disabled={saving || extracting || (kind === "client" && clients.length === 0)} className="rounded-lg px-5 py-2.5 text-sm font-semibold disabled:opacity-60" style={{ ...UI.primary, color: "#06213a" }}>{saving ? "Saving…" : kind === "client" ? "Create invoice" : "Create bill"}</button></div>
      </form>
    </section>
  </div>;
}
