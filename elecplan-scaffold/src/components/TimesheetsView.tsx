...
export default function TimesheetsView({ entries, role }: { entries: TimesheetRow[]; role: Role; currentUserId: string }) {
  ...
  return <>
    <TopBar title="Timesheets" subtitle="Track weekly submitted and approved hours" rightSlot={<button type="button" onClick={() => setShowNew(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ background: UI.blue, color: "white" }}><Plus size={16} /> Add hours</button>} />
