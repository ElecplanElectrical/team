...
const UI = { panel: "#0a2038", panelAlt: "#103152", border: "rgba(125,211,252,.28)", borderSoft: "rgba(125,211,252,.14)", text: "#f5f9ff", mute: "#a8c3dd", faint: "#7392af", blue: "#38bdf8", cyan: "#7dd3fc", green: "#18d3a0", orange: "#ffb454", red: "#ff7185" };
...
export default function TimesheetsView({ entries, role }: { entries: TimesheetRow[]; role: Role; currentUserId: string }) {
  ...
  return <>
    <TopBar title="Timesheets" subtitle="Track weekly submitted and approved hours" rightSlot={<button type="button" onClick={() => setShowNew(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ background: UI.blue, color: "#06213a" }}><Plus size={16} /> Add hours</button>} />
...
function NewTimesheet({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const [weekStart, setWeekStart] = useState(""); const [hours, setHours] = useState("38"); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null); const field = { background: "#041323", border: `1px solid ${UI.border}`, color: UI.text } as const;
