...
export default function TimesheetsView({ entries, role }: { entries: TimesheetRow[]; role: Role; currentUserId: string }) {
  ...
  return <>
    <TopBar title="Timesheets" subtitle="Track weekly submitted and approved hours" rightSlot={<button type="button" onClick={() => setShowNew(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ ...UI.primary, color: "#06213a" }}><Plus size={16} /> Add hours</button>} />
...
function NewTimesheet({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const [weekStart, setWeekStart] = useState(""); const [hours, setHours] = useState("38"); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null); const field = { ...{boxShadow:"var(--ep-inset-shadow)"}, background: "var(--ep-input)", border: `1px solid ${UI.border}`, color: UI.text } as const;
