...
function statusStyle(status: string) {
  ...
  if (status === "SCHEDULED") return { bg: "rgba(67,210,255,.12)", fg: UI.cyan };
...
export default function ReelsView({ ideas }: { ideas: Idea[] }) {
  ...
  return <>
    <TopBar title="Reels & content" subtitle="Plan content without connecting social accounts" rightSlot={<button type="button" onClick={() => setOpen(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ ...UI.primary, color: UI.activeText }}><Plus size={16} /> New content idea</button>} />
