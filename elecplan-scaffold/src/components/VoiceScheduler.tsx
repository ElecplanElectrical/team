    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/65 p-0 backdrop-blur-sm md:items-center md:p-5" onClick={onClose}>
      <section className="w-full max-w-xl rounded-t-2xl p-5 shadow-2xl md:rounded-2xl" style={{ background: UI.bg, border: `1px solid ${UI.border}` }} onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: "rgba(67,210,255,.14)", color: UI.cyan, border: "1px solid rgba(67,210,255,.22)" }}><Mic size={21} /></div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2"><h2 className="text-base font-semibold" style={{ color: UI.text }}>Voice scheduler</h2><span className="rounded-full px-2 py-0.5 text-[9px] font-semibold" style={{ color: UI.cyan, background: "rgba(67,210,255,.12)" }}>BETA</span></div>
        ...
        <button type="button" onClick={startListening} className="mx-auto mt-6 flex h-24 w-24 items-center justify-center rounded-full transition active:scale-95" style={{ background: listening ? "radial-gradient(circle,#43D2FF,#43D2FF)" : "radial-gradient(circle,#43D2FF,#43D2FF)", color: "white", boxShadow: listening ? "0 0 0 12px rgba(67,210,255,.10),0 0 50px rgba(67,210,255,.45)" : "0 0 35px rgba(67,210,255,.25)" }}>
        ...
        <div className="mt-5">
          ...
          <textarea data-voice-schedule-input value={transcript} onChange={(event) => setTranscript(event.target.value)} rows={3} placeholder="Schedule a job tomorrow at 9am with Jack for 2 hours" className="w-full resize-none rounded-xl px-3 py-3 text-sm outline-none" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}`, color: UI.text }} />
          <button type="button" onClick={() => void preview()} disabled={!transcript.trim()} className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold disabled:opacity-40" style={{ background: "rgba(67,210,255,.14)", color: UI.cyan, border: "1px solid rgba(67,210,255,.25)" }}><Send size={14} /> Interpret request</button>
        </div>

        {proposal && (
          <div className="mt-4 rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}>
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
              {proposal.type === "job" && proposal.jobId && role !== "EMPLOYEE" && <button type="button" disabled={saving} onClick={() => void create(true)} className="rounded-lg px-4 py-2.5 text-sm font-semibold" style={{ color: UI.cyan, border: "1px solid rgba(67,210,255,.35)" }}>Create & text client</button>}
              <button type="button" disabled={saving} onClick={() => void create(false)} className="rounded-lg px-4 py-2.5 text-sm font-semibold" style={{ ...UI.primary, color: UI.activeText }}>{saving ? "Scheduling…" : conflicts.length ? "Schedule anyway" : "Confirm schedule"}</button>
            </div>
          </div>
        )}
