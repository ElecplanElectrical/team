...
export default function EmpireDocumentsDemoClient(){
 ...
 {selected&&<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4"><div className="w-full max-w-lg rounded-2xl border p-5" style={{background:p,borderColor:b}}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-widest" style={{color:g}}>{selected.folder}</p><h2 className="mt-2 text-xl font-black">{selected.name}</h2></div><button onClick={()=>setSelected(null)} className="rounded-lg border p-2" style={{borderColor:b,color:m}} aria-label="Close preview"><X size={16}/></button></div><div className="mt-5 rounded-xl border p-5" style={{borderColor:b,background:"#0c0f10"}}><FileText size={28} style={{color:g}}/><p className="mt-4 text-sm leading-6" style={{color:m}}>Secure document preview. Team files remain grouped with the event, rider or operational record they belong to.</p></div><button onClick={()=>download(selected)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-bold" style={{background:g,color:"#061008"}}><Download size={15}/>Download file</button></div></div>}
 </main>
}
