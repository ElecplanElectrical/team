...
const UI = { panel: "var(--brand-panel, #07192b)", panelAlt: "var(--brand-panel-alt, #09213a)", border: "var(--brand-border, rgba(77,150,221,.24))", borderSoft: "var(--brand-border-soft, rgba(77,150,221,.12))", text: "#f5f9ff", mute: "var(--brand-muted, #93a9c2)", faint: "var(--brand-faint, #617993)", blue: "var(--brand-primary, #168dff)", cyan: "var(--brand-accent, #25c7ff)", green: "#18d3a0", red: "#ff5e72", orange: "#ff9f1c" };
...
function statusStyle(status: Status) {
  ...
  return { bg: "rgb(var(--brand-primary-rgb, 22 141 255) / .11)", fg: UI.cyan, border: "rgb(var(--brand-accent-rgb, 37 199 255) / .24)" };
}
...
function NewInspection({ jobs, onClose, onDone }: { jobs: Job[]; onClose: () => void; onDone: () => void }) {
  const [jobId, setJobId] = useState(""); const [type, setType] = useState("Electrical inspection"); const [date, setDate] = useState(""); const [status, setStatus] = useState<Status>("SCHEDULED"); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null); const field = { background: "var(--brand-panel-deep, #041323)", border: `1px solid ${UI.border}`, color: UI.text } as const;
