...
function statusStyle(status: Status) {
  ...
  return { bg: "rgba(67,210,255,.11)", fg: UI.cyan, border: "rgba(67,210,255,.24)" };
}
...
export default function InspectionsView({ inspections, jobs }: { inspections: InspectionRow[]; jobs: Job[] }) {
  ...
  return <>
    <TopBar title="Inspections" subtitle="Schedule and track compliance inspections" rightSlot={<button type="button" onClick={() => setShowNew(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ ...UI.primary, color: UI.activeText }}><Plus size={16} /> New inspection</button>} />
...
function NewInspection({ jobs, onClose, onDone }: { jobs: Job[]; onClose: () => void; onDone: () => void }) {
  const [jobId, setJobId] = useState(""); const [type, setType] = useState("Electrical inspection"); const [date, setDate] = useState(""); const [status, setStatus] = useState<Status>("SCHEDULED"); const [saving, setSaving] = useState(false); const [error, setError] = useState<string | null>(null); const field = { ...{boxShadow:"var(--ep-inset-shadow)"}, background: "var(--ep-input)", border: `1px solid ${UI.border}`, color: UI.text } as const;
