...
export default function KpisView({ employees, initialKpis }: { employees: Employee[]; initialKpis: Kpi[] }) {
  ...
  const field = { ...{boxShadow:"var(--ep-inset-shadow)"}, background: "var(--ep-input)", border: `1px solid ${UI.border}`, color: UI.text };
  ...
  return <>
    ...
    <main className="flex-1 overflow-auto p-4 md:p-6" style={{ background: "var(--ep-main)" }}>
      <div className="mx-auto max-w-5xl space-y-4">
        <section className="rounded-xl p-4 md:p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}>
          ...
          <div className="mt-4 flex justify-end"><button onClick={save} disabled={saving} className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold" style={{ ...UI.primary, color: "#06213a" }}><Plus size={15} />{saving ? "Saving…" : "Save week"}</button></div>
        </section>

        <section className="rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}>
