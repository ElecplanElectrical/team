...
function MobileBrand({ brand, drawer = false }: { brand?: Brand; drawer?: boolean }) {
  ...
  return <div className="flex items-center gap-2.5"><Image unoptimized src={brand.logoUrl} alt={brand.name} width={300} height={220} className={drawer ? "h-28 w-auto max-w-[150px] object-contain object-left" : "h-11 w-11 object-contain"} /></div>;
}
...
export default function MobileNav({ role, brand }: { role: Role; brand?: Brand }) {
  ...
  const groups = navGroupsFor(role, brand?.modules)
    .map(group => ({ ...group, items: isQls ? group.items.filter(item => item.screen !== "inspections") : group.items }))
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[60] md:hidden">
      ...
      <aside className="absolute inset-y-0 left-0 flex w-[84%] max-w-[340px] flex-col overflow-hidden p-4 shadow-2xl" style={{ background: isQls ? `linear-gradient(180deg,#050806,${primary}08,#030504)` : "linear-gradient(180deg,#02101f,#031321)", borderRight: `1px solid ${border}` }}>
        {isQls && <div className="pointer-events-none absolute inset-x-0 top-[118px] bottom-[78px] overflow-hidden" aria-hidden="true">
          <Image src="/qls-logo-transparent.svg" alt="" width={241} height={282} className="absolute left-1/2 top-[44%] w-[325px] max-w-none -translate-x-1/2 -translate-y-1/2 grayscale opacity-[0.05]" />
        </div>}

        <div className="relative z-10 mb-5 flex items-center justify-between">
          <MobileBrand brand={brand} drawer />
          <button type="button" onClick={() => setOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ background: panel, border: `1px solid ${border}`, color: UI.mute }}><X size={18} /></button>
        </div>

        <nav className="relative z-10 min-h-0 flex-1 space-y-5 overflow-y-auto pr-1">
          {groups.map((group, gi) => <div key={group.heading ?? gi}>
            {group.heading && <p className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-[0.13em]" style={{ color: UI.faint }}>{group.heading}</p>}
            <div className="space-y-1">
              {group.items.map(item => {
                const href = SCREEN_PATH[item.screen];
                const active = pathname === href || pathname.startsWith(href + "/");
                const Icon = item.icon;
                return <Link key={item.screen} href={href} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium" style={{ background: active ? `linear-gradient(90deg,${primary}b8,${primary}35)` : "transparent", color: active ? "#f4f8ff" : UI.mute }}><Icon size={17} style={{ color: active ? accent : undefined }} />{item.label}</Link>;
              })}
            </div>
          </div>)}
        </nav>

        <button type="button" onClick={() => signOut({ callbackUrl: logoutUrl })} className="relative z-10 mt-4 flex items-center gap-3 border-t px-3 pt-5 text-sm font-medium" style={{ borderColor: border, color: UI.mute }}><LogOut size={17} /> Sign out{isQls ? " of QLS" : ""}</button>
      </aside>
    </div>}
  </>;
}
