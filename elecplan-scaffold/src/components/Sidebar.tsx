  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto pr-0.5">
        {groups.map((group, gi) => (
          <div key={group.heading ?? gi} className="mb-4 flex flex-col gap-1">
            {group.heading && (
              <p className="hidden px-3 pb-1 text-[10px] font-semibold uppercase tracking-[0.14em] xl:block" style={{ color: UI.faint }}>
                {group.heading}
              </p>
            )}
            {group.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  title={item.label}
                  className="group flex h-10 items-center justify-center gap-3 rounded-lg px-2 text-sm font-medium transition xl:justify-start xl:px-3"
                  style={{
      ...
      <div className="mt-4 border-t pt-3" style={{ borderColor: UI.border }}>
        <div className="flex items-center justify-center gap-2 rounded-xl px-1 py-2 xl:justify-start xl:px-2" style={{ background: "rgba(8,28,48,.72)" }}>
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-semibold" style={{ background: "#0b2b4c", border: `1px solid rgba(37,199,255,.28)`, color: "#d9f5ff" }}>
            {initialsOf(name)}
          </div>
          <div className="hidden min-w-0 flex-1 xl:block">
            <p className="truncate text-xs font-semibold" style={{ color: UI.text }}>{name}</p>
            <p className="truncate text-[10px]" style={{ color: UI.faint }}>{ROLE_TITLE[role]}</p>
          </div>
          <ChevronDown size={13} className="hidden xl:block" style={{ color: UI.faint }} />
        </div>
        <div className="mt-2 hidden items-center gap-1 xl:flex">
          <Link href={role === "ADMIN" ? "/settings" : "/account"} className="flex flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-[11px]" style={{ color: UI.faint }}>
            <Settings size={13} /> Settings
          </Link>
          <button type="button" onClick={() => signOut({ callbackUrl: "/login" })} className="rounded-md p-1.5" style={{ color: UI.faint }} aria-label="Sign out">
            <LogOut size={13} />
          </button>
        </div>
      </div>
