...
export default function DocumentsView({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/documents", {
      ...
      body: JSON.stringify({ name, type, fileUrl, jobId: jobId || null }),
    });
    setSaving(false);
    if (!res.ok) {
    ...
    <>
      ...
      <div className="flex-1 overflow-auto p-4 md:p-8 flex flex-col gap-4">
        <div className="rounded-lg px-4 py-3 text-sm" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, color: COLORS.textMute }}>
          Managed uploads are intentionally not enabled yet. Register HTTPS links from your approved storage provider here; private R2/S3 uploads will be added after storage credentials and access rules are configured.
        </div>
        ...
        {showForm && (
          <form onSubmit={submit} className="rounded-lg p-4 grid grid-cols-1 md:grid-cols-2 gap-3" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
            ...
            <div className="md:col-span-2 flex justify-end gap-2">
              ...
              <button disabled={saving} className="rounded-md px-4 py-2 text-sm font-semibold disabled:opacity-60" style={{ background: COLORS.accent, color: ON_ACCENT }}>{saving ? "Adding…" : "Add document"}</button>
            </div>
          </form>
        )}
        ...
        <div className="rounded-lg overflow-hidden" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}` }}>
          ...
          {documents.length === 0 && <div className="px-5 py-10 text-center text-sm" style={{ color: COLORS.textFaint }}>No documents registered yet.</div>}
        </div>
      </div>
    </>
  );
