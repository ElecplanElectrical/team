      ...
      <section className="flex items-center justify-center p-5 md:p-8" style={{background:"radial-gradient(circle at 50% 10%,rgba(105,190,40,.08),transparent 28%)"}}>
        <div className="w-full max-w-md">
          ...
          <form onSubmit={submit} className="rounded-2xl border p-6 shadow-2xl md:p-8" style={{background:"rgba(17,20,22,.97)",borderColor:"rgba(105,190,40,.42)",boxShadow:"0 24px 80px rgba(0,0,0,.45)"}}>
            <div className="text-center"><p className="text-xs font-black uppercase tracking-[.28em]" style={{color:g}}>Welcome to</p><h2 className="mt-2 text-4xl font-black uppercase">Empire HQ</h2><p className="mt-2 text-sm" style={{color:m}}>Presentation sign-in preview</p></div>
            <div className="mt-6 flex items-start gap-2 rounded-lg border p-3 text-xs" style={{borderColor:b,background:"#0d1011",color:m}}><ShieldCheck size={15} style={{color:g}}/><span>This preview behaves like the private team entry screen without exposing a real customer login.</span></div>
