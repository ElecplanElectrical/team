...
import { AlertTriangle, Minus, PackageSearch, Plus, Search, X } from "lucide-react";
...
function NewStockItem({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => null);
        setError(body?.error ?? "Could not create stock item.");
        return;
      }
      onDone();
    } catch {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-0 backdrop-blur-sm md:items-center md:p-4" onClick={onClose} role="presentation"><section className="w-full max-w-lg overflow-hidden rounded-t-2xl md:rounded-2xl" style={{ background: UI.panel, border: `1px solid ${UI.border}`, boxShadow: "0 28px 90px rgba(0,0,0,.35)" }} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="new-stock-title"><div className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: UI.borderSoft }}><div><h2 id="new-stock-title" className="font-semibold" style={{ color: UI.text }}>New stock item</h2><p className="mt-1 text-xs" style={{ color: UI.faint }}>Add a material and its reorder level.</p></div><button type="button" aria-label="Close" onClick={onClose} className="p-1" style={{ color: UI.mute }}><X size={18} /></button></div><form onSubmit={submit} className="flex max-h-[82vh] flex-col gap-3 overflow-auto p-5" aria-busy={saving}><Field title="Name"><input required value={name} onChange={(e) => setName(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm outline-none" style={field} autoFocus /></Field><div className="grid grid-cols-2 gap-3"><Field title="Unit"><input required value={unit} onChange={(e) => setUnit(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm outline-none" style={field} /></Field><Field title="Supplier"><input value={supplier} onChange={(e) => setSupplier(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm outline-none" style={field} /></Field></div><div className="grid grid-cols-2 gap-3"><Field title="On hand"><input type="number" inputMode="numeric" min="0" required value={onHand} onChange={(e) => setOnHand(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm outline-none" style={field} /></Field><Field title="Par level"><input type="number" inputMode="numeric" min="0" required value={parLevel} onChange={(e) => setParLevel(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm outline-none" style={field} /></Field></div>{error && <div role="alert" className="flex gap-2 rounded-lg p-3 text-xs leading-5" style={{ background: "rgba(255,94,114,.08)", border: "1px solid rgba(255,94,114,.22)", color: UI.red }}><AlertTriangle size={14} className="mt-0.5 shrink-0" /><span>{error}</span></div>}<div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="rounded-lg px-4 py-2.5 text-sm" style={{ background: UI.panelAlt, color: UI.mute, border: `1px solid ${UI.borderSoft}` }}>Cancel</button><button type="submit" disabled={saving} className="rounded-lg px-4 py-2.5 text-sm font-semibold disabled:opacity-60" style={{ background: UI.blue, color: "white" }}>{saving ? "Saving…" : "Create item"}</button></div></form></section></div>;
}
