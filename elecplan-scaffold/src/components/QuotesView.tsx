...
const STATUS_STYLE: Record<QuoteStatus, { bg: string; fg: string; border: string }> = {
  ...
  SENT: { bg: "rgba(67,210,255,.12)", fg: "#78e5ff", border: "rgba(67,210,255,.30)" },
...
export default function QuotesView({ quotes, clients, jobs }: { quotes: QuoteRow[]; clients: QuoteClientOption[]; jobs: QuoteJobOption[] }) {
  ...
  return (
    <>
      <TopBar title="Quotes" subtitle="Create and manage quotes" rightSlot={<button type="button" onClick={() => setShowNew(true)} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ ...UI.primary, color: UI.activeText, boxShadow: "0 8px 24px rgba(67,210,255,.25)" }}><Plus size={16} /> New quote</button>} />
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="text-[11px]" style={{ color: UI.faint }}>{label}</div><div className="mt-1 text-xl font-semibold" style={{ color: UI.text }}>{value}</div></div>;
}
