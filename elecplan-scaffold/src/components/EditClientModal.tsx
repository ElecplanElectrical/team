...
export default function EditClientModal({ client, onClose, onDone }: { client: EditableClient; onClose: () => void; onDone: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-0 backdrop-blur-sm md:items-center md:p-4" onClick={onClose} role="presentation">
    <section className="w-full max-w-xl overflow-hidden rounded-t-2xl md:rounded-2xl" style={{ background: UI.panel, border: `1px solid ${UI.border}`, boxShadow: "0 28px 90px rgba(0,0,0,.35)" }} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="edit-client-title">
      <header className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: UI.borderSoft }}>
        ...
        <div className="min-w-0 flex-1"><h2 id="edit-client-title" className="text-base font-semibold" style={{ color: UI.text }}>Edit client</h2><p className="mt-1 truncate text-xs" style={{ color: UI.faint }}>{client.name}</p></div>
      ...
      <form onSubmit={submit} className="max-h-[82vh] overflow-auto p-5" aria-busy={saving}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Email"><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 w-full rounded-lg px-3 text-sm outline-none" style={field} autoCapitalize="none" spellCheck={false} /></Field>
