...
import { CalendarDays, ChevronLeft, ChevronRight, Filter, Mic, Plus, Users } from "lucide-react";
      ...
      <div className="flex-1 overflow-auto p-3 md:p-4 xl:p-5" style={{ background: "radial-gradient(circle at 55% 0%,rgba(20,91,160,.13),transparent 35%),#03101f" }}>
        <div className="mx-auto grid w-full max-w-[1700px] gap-3 xl:grid-cols-[190px_minmax(0,1fr)_280px]">
          ...
          <aside className="hidden xl:block rounded-xl p-4" style={{ background: UI.panel, border: `1px solid ${UI.border}` }}>
            {editingEvent ? <><div className="flex items-center justify-between"><h2 className="text-sm font-semibold" style={{ color:UI.text }}>Event details</h2><span className="rounded-full px-2 py-1 text-[9px] capitalize" style={{ background:'rgba(22,141,255,.13)', color:UI.cyan }}>{editingEvent.type}</span></div><h3 className="mt-5 text-base font-semibold" style={{ color:UI.text }}>{editingEvent.title}</h3><p className="mt-2 text-xs leading-5" style={{ color:UI.mute }}>{format(parseISO(editingEvent.startsAt),'EEE d MMM yyyy')}<br />{timeRange(editingEvent.startsAt,editingEvent.endsAt)}</p><button type="button" onClick={() => setEditingEvent(editingEvent)} className="mt-6 w-full rounded-lg px-3 py-2.5 text-xs font-semibold" style={{ background:UI.blue,color:'white' }}>Edit event</button></> : <div className="flex min-h-[320px] flex-col items-center justify-center text-center"><CalendarDays size={28} style={{ color:UI.faint }} /><p className="mt-3 text-sm font-semibold" style={{ color:UI.text }}>Select an event</p><p className="mt-1 max-w-[190px] text-xs leading-5" style={{ color:UI.mute }}>Job and appointment details will appear here.</p></div>}
          </aside>
        </div>
      </div>
