...
const UI={panel:"var(--brand-panel, #07192b)",panelAlt:"var(--brand-panel-alt, #09213a)",border:"var(--brand-border, rgba(77,150,221,.24))",borderSoft:"var(--brand-border-soft, rgba(77,150,221,.12))",text:"#101828",mute:"var(--brand-muted, #667085)",faint:"var(--brand-faint, #8b97a3)",blue:"var(--brand-primary, #168dff)",cyan:"var(--brand-accent, #25c7ff)"};
const JOB_CALENDAR_COLOR:Record<string,{bg:string;fg:string;border:string}>={
 SCHEDULED:{bg:"rgba(70,199,205,.20)",fg:"#155e66",border:"#43b6bb"},
 IN_PROGRESS:{bg:"rgba(66,207,112,.20)",fg:"#17653a",border:"#42cf70"},
 COMPLETE:{bg:"rgba(96,165,250,.20)",fg:"#24558f",border:"#60a5fa"},
};
...
export default function CalendarView({weekStart,events,jobs,employees,role,currentUserId}:{weekStart:string;events:CalendarEvent[];jobs:JobOption[];employees:{id:string;name:string}[];role:Role;currentUserId:string}){
 ...
 function weekBar(segment:WeekSegment){
  ...
  const textColor=job?.status?c.fg:"#344054";
  const continuesBefore=dayStamp(parseISO(event.startsAt))<dayStamp(weeks.flat()[0]??monthStart);
  void continuesBefore;
  return <button key={`${event.id}-${format(weeks[0]?.[0]??monthStart,"yyyy-MM")}-${lane}-${startCol}`} onClick={()=>openEvent(event)} className="pointer-events-auto min-w-0 truncate px-2.5 py-1 text-left text-[10px] font-semibold sm:text-[11px]" style={{gridColumn:`${startCol+1} / ${endCol+2}`,gridRow:lane+1,background:c.bg,color:textColor,borderLeft:`3px solid ${c.border}`,borderRight:`2px solid ${c.border}`,borderTop:`1px solid ${soft(c.border,48)}`,borderBottom:`1px solid ${soft(c.border,48)}`,borderRadius:7,marginLeft:5,marginRight:5,height:25,alignSelf:"start",textShadow:"none"}} title={`${label} — ${format(parseISO(event.startsAt),"d MMM")} to ${format(parseISO(event.endsAt),"d MMM")}`}>{label}</button>
 }
