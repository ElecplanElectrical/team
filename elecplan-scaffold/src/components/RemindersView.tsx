...
export default function RemindersView({ reminders }: { reminders: Task[] }) {
  ...
  return <>
    <TopBar title="To Do List" subtitle="Tasks to get done" rightSlot={<button type="button" onClick={() => { setShowForm(value => !value); setEditing(null); }} className="flex h-10 items-center gap-2 rounded-lg px-3.5 text-sm font-semibold" style={{ background: UI.blue, color: "white" }}><Plus size={16}/> New task</button>}/>
