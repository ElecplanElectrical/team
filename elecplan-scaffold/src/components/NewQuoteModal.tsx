...
export default function NewQuoteModal({ clients, jobs, onClose, onDone }: { clients: QuoteClientOption[]; jobs: QuoteJobOption[]; onClose: () => void; onDone: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-0 backdrop-blur-sm md:items-center md:p-4" onClick={onClose}>
    <section className="w-full max-w-3xl overflow-hidden rounded-t-2xl md:rounded-2xl" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}`, boxShadow: "0 28px 90px rgba(0,0,0,.35)" }} onClick={(e) => e.stopPropagation()}>
      <header className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: UI.borderSoft }}>
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "rgba(67,210,255,.11)", color: UI.cyan }}><FileText size={18} /></span>
      ...
      <form onSubmit={submit} className="max-h-[84vh] overflow-auto p-5">
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between gap-3"><span className="text-[10px] font-semibold uppercase tracking-[.12em]" style={{ color: UI.faint }}>Line items</span><button type="button" onClick={() => setLines((current) => [...current, blankLine()])} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold" style={{ background: "rgba(67,210,255,.10)", color: UI.cyan, border: "1px solid rgba(67,210,255,.22)" }}><Plus size={13} /> Add line</button></div>
          <div className="space-y-2">
            {lines.map((line, index) => <div key={index} className="grid grid-cols-1 gap-3 rounded-xl p-3 sm:grid-cols-[minmax(180px,1fr)_90px_130px_110px_38px] sm:items-end" style={{ ...UI.inset, background: UI.panelAlt, border: `1px solid ${UI.borderSoft}` }}>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="rounded-lg px-4 py-2.5 text-sm font-semibold" style={{ ...UI.inset, background: UI.panelAlt, color: UI.mute, border: `1px solid ${UI.borderSoft}` }}>Cancel</button><button type="submit" disabled={saving || clients.length === 0} className="rounded-lg px-5 py-2.5 text-sm font-semibold disabled:opacity-60" style={{ ...UI.primary, color: UI.activeText }}>{saving ? "Saving…" : "Create quote"}</button></div>
      </form>
    </section>
  </div>;
}
