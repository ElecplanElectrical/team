...
function Metric({ icon, label, value, hint }: { icon: React.ReactNode; label: string; value: string; hint: string }) { return <div className="rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="flex items-center gap-2 text-[11px]" style={{ color: UI.faint }}><span style={{ color: UI.cyan }}>{icon}</span>{label}</div><div className="mt-2 text-2xl font-semibold" style={{ color: UI.text }}>{value}</div><div className="mt-1 text-xs" style={{ color: UI.mute }}>{hint}</div></div>; }
function Panel({ title, href, linkLabel, children }: { title: string; href: string; linkLabel: string; children: React.ReactNode }) { return <div className="rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="mb-4 flex items-center justify-between gap-3"><h2 className="text-sm font-semibold" style={{ color: UI.text }}>{title}</h2><Link className="text-xs font-semibold" style={{ color: UI.cyan }} href={href}>{linkLabel}</Link></div><div className="space-y-3">{children}</div></div>; }
...
function Progress({ label, value }: { label: string; value: number }) { const bounded = Math.max(0, Math.min(100, value)); return <div><div className="mb-1.5 flex justify-between text-xs"><span style={{ color: UI.mute }}>{label}</span><span style={{ color: UI.text }}>{pct(bounded)}</span></div><div className="h-2 overflow-hidden rounded-full" style={{ ...UI.inset, background: UI.panelAlt }}><div className="h-full rounded-full" style={{ width: `${bounded}%`, ...UI.primary }} /></div></div>; }
function ProgressCard({ title, value, href }: { title: string; value: number; href: string }) { return <Link href={href} className="block rounded-xl p-5" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><div className="mb-3 text-sm font-semibold" style={{ color: UI.text }}>{title}</div><Progress label="Performance" value={value} /></Link>; }
