      ...
      <div className="flex-1 overflow-auto p-3 md:p-4 xl:p-5" style={{ background: "var(--ep-main)" }}>
        <div className="mx-auto w-full max-w-[1700px] space-y-3">
          ...
          <section className="overflow-hidden rounded-xl" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}>
            <div className="border-b p-3" style={{ borderColor: UI.borderSoft }}><div className="relative max-w-md"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: UI.faint }} /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search employees…" className="h-10 w-full rounded-lg pl-9 pr-3 text-sm outline-none" style={{ ...{boxShadow:"var(--ep-inset-shadow)"}, background: "var(--ep-input)", color: UI.text, border: `1px solid ${UI.border}` }} /></div></div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl p-4" style={{ ...UI.raised, background: UI.panel, border: `1px solid ${UI.border}` }}><p className="text-[11px]" style={{ color: UI.faint }}>{label}</p><p className="mt-1 text-xl font-semibold" style={{ color: UI.text }}>{value}</p></div>;
}
