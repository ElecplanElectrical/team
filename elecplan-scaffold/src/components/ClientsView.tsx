...
function SmallMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg p-3" style={{ background: UI.panelAlt, border: `1px solid ${UI.borderSoft}` }}><p className="text-[10px]" style={{ color: UI.faint }}>{label}</p><p className="mt-1 text-sm font-semibold" style={{ color: UI.text }}>{value}</p></div>;
}

function Detail({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex gap-3"><span className="mt-0.5" style={{ color: UI.cyan }}>{icon}</span><div className="min-w-0"><p style={{ color: UI.faint }}>{label}</p><p className="mt-0.5 break-words leading-5" style={{ color: UI.text }}>{value}</p></div></div>;
}
