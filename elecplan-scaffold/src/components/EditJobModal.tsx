...
export default function EditJobModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!title.trim() || !address.trim()) {
      setError("Job title and address are required.");
      return;
    }
    if (Boolean(scheduledStart) !== Boolean(scheduledEnd)) {
      setError("Add both scheduled start and end, or clear both.");
      return;
    }
    ...
    if (!res.ok) {
      ...
      setError(body?.error ?? "Could not update the job.");
      return;
    }
    onDone();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="w-full max-w-lg rounded-lg overflow-hidden" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}` }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${COLORS.borderSoft}` }}>
          <div>
            <h2 className="text-base font-semibold" style={{ fontFamily: FONTS.display, color: COLORS.text }}>Edit job</h2>
            <p className="text-xs mt-0.5" style={{ color: COLORS.textFaint }}>{job.ref} · {job.client}</p>
          </div>
        ...
        <form onSubmit={submit} className="p-5 flex flex-col gap-3 max-h-[80vh] overflow-auto">
          <Field label="Job title">
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-md px-3 py-2 text-sm outline-none" style={fieldStyle} autoFocus />
          </Field>
          ...
          <p className="text-[11px] -mt-1" style={{ color: COLORS.textFaint }}>Changing these times updates the linked Calendar event. Clearing both removes that scheduled event.</p>

          <Field label="Notes">
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={4} className="w-full rounded-md px-3 py-2 text-sm outline-none resize-none" style={fieldStyle} />
          </Field>
          ...
          <div className="flex justify-end gap-2 mt-1">
            ...
            <button type="submit" disabled={saving} className="rounded-md px-4 py-2 text-sm font-semibold disabled:opacity-60" style={{ background: COLORS.accent, color: ON_ACCENT }}>{saving ? "Saving…" : "Save changes"}</button>
          </div>
        </form>
      </div>
    </div>
